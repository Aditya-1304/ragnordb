...
use ragnordb_storage::wal::{DurableCommitLog, DurableWalExtent, SingleNodeTxnCommit, WalMutation};
use ragnordb_tablet::{
    command::{TabletCommandApplyError, TabletCommandApplyOutcome},
    snapshot::{FileTabletSnapshotStore, TabletSnapshotInstallTarget},
};
use tracing::{error, info, warn};
use wal::{io::directory::FsSegmentDirectory, lsn::Lsn, wal::WalHandle};

use crate::{config::NodeConfig, database::SharedLocalDatabase};
...
impl ReplicatedTabletHandle {
    ...
    pub fn is_leader(&self) -> bool {
        self.status()
            .leader_replica_id
            .is_some_and(|leader| leader == self.local_replica_id)
    }
...
impl ReplicatedTabletRuntime {
    ...
    pub fn start(
        ...
        let snapshot_store = FileTabletSnapshotStore::new(
    ...
    SS: StableStore,
{
    ...
    if let Some(ready) = initial_ready {
        send_messages(&transport, ready.messages);
    }
    ...
    while !shutdown.load(Ordering::Acquire) {
        ...
        publish_status(&ready_loop, &status);
